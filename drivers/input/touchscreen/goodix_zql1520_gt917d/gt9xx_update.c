...
//*****************************Add Factory Mode node**************************
static char fw_verion_sw[30] ={"CTPFW_GT917D_YJ_03.1050.43"};
static struct kobject *firmware_ver_kobj_sw = NULL;

static ssize_t gtp_fw_ver_show(struct kobject *kobj, struct kobj_attribute *attr, char *buff)
{
    ssize_t len = 0;
    len +=sprintf(buff, "%s\n",fw_verion_sw);
    return len;
...
static struct kobj_attribute firmware_attr = __ATTR(fw_verion_sw, 0444, gtp_fw_ver_show, NULL);

int gtp_firmware_sysfs(void)
{
    int ret = 0;

    firmware_ver_kobj_sw = kobject_create_and_add("ctp_fw", kernel_kobj);
    if (firmware_ver_kobj_sw == NULL) {
        ret = -ENOMEM;
        GTP_ERROR("register sysfs ctp_fw failed. ret = %d\n", ret);
        return ret;
    }

    ret = sysfs_create_file(firmware_ver_kobj_sw, &firmware_attr.attr);
    if (ret) {
        ret = -ENOMEM;
        GTP_ERROR("create fw_verion_sw failed. ret = %d\n", ret);
        return ret;
    }

    GTP_INFO("gtp_firmware_sysfs success\n");
    return ret;
}
#endif
