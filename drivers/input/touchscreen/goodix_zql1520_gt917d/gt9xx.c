...
u8 config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    ...
    extern int gtp_firmware_sysfs(void);
#endif
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        if (ret > 0)
        {
	printk("GTP Driver send config err\n");
            break;
        }
    }
#endif
...
#if GTP_ICS_SLOT_REPORT
    /* Modified by luochuan for protocol B 20170620 begin */
    input_mt_init_slots(ts->pen_dev, GTP_MAX_TOUCH, 0);               // 
    /* Modified by luochuan for protocol B 20170620 end */
#else
...
s32 gtp_hq_hardinfo(struct i2c_client *client)
{
    ...
    if(vendor_id_hw[2] == 0x00) {
        /* YJ vendor id is 0x03 in huaqin but 0x00 in goodix */
        vendor_id_hw[2] = 0x03;
    } else {
        /* vendor id error */
        vendor_id_hw[2] = 0xff;
    ...
    sprintf(gtp_tpdname,"CTPFW_GT917D_YJ_%02x.%02x%02x.%02x", vendor_id_hw[2], fw_ver_hw[7],
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    ...
    hq_regiser_hw_info(HWID_CTP, gtp_tpdname);
