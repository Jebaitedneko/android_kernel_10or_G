...
int ipa_rm_delete_resource(enum ipa_rm_resource_name resource_name)
{
	...
	if (resource->state == IPA_RM_GRANTED) {
		/* There might pending timer work to release release the resource */
...
int ipa_rm_request_resource_with_timer(enum ipa_rm_resource_name resource_name)
{
	...
	resource->release_work = kzalloc(sizeof(*resource->release_work), GFP_ATOMIC);
	...
	INIT_DELAYED_WORK(&resource->release_work->work, delayed_release_work_func);
